...
private[mappings] class LocalDateFormatter(key: String) extends Formatter[Option[LocalDate]] {
  ...
  val earliestEffectiveDate = LocalDate.of(1900, 1, 1)
  val yearMsg = s"$key.year"
  val monthMsg = s"$key.month"
  val dayMsg = s"$key.day"
  val invalidDate = s"$key.error.invalidDate"
  val dayMonthRequired = s"$key.error.mandatory.dayMonth"
  val dayYearRequired = s"$key.error.mandatory.dayYear"
  val monthYearRequired =s"$key.error.mandatory.monthYear"
  val dayRequired = s"$key.error.mandatory.day"
  val monthRequired = s"$key.error.mandatory.month"
  val yearRequired = s"$key.error.mandatory.year"
  val yearRange = s"$key.error.year.range"
  val dayRange = s"$key.error.day.range"
  val dayNumber = s"$key.error.day.number"
  val monthRange = s"$key.error.month.range"
  val monthNumber = s"$key.error.month.number"
  val yearNumber = s"$key.error.year.number"
