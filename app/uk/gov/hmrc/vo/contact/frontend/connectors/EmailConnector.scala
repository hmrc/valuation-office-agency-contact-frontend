...
import uk.gov.hmrc.vo.contact.frontend.models.{Contact, ContactWithEnMessage}
import uk.gov.hmrc.vo.contact.frontend.models.requests.DataRequest
import uk.gov.hmrc.vo.contact.frontend.utils.DateUtil
...
class EmailConnector @Inject() (
  ...
  private def getEnquirySubject(contact: Contact, isUpdateExistingEnquiry: Boolean)(using messages: Messages): String =
  ...
  private def getNextStepText(isUpdateExistingEnquiry: Boolean)(using messages: Messages): String =
  ...
  private def sendEmail(email: String, templateId: String, parametersJson: JsObject)(using hc: HeaderCarrier): Future[HttpResponse] =
    val json = Json.obj(
      "to"         -> Seq(email),
