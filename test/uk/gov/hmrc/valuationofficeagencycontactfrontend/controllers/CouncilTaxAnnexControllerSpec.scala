...
class CouncilTaxAnnexControllerSpec extends ControllerSpecBase {
  ...
  def controller(dataRetrievalAction: DataRetrievalAction = getEmptyCacheMap) =
    new CouncilTaxAnnexController(frontendAppConfig, messagesApi, councilTaxAnnex,
      MessageControllerComponentsHelpers.stubMessageControllerComponents)

  "Council Tax Annex Controller" must {
    "return the correct view for a GET" in {
      val result = controller().onPageLoad()(fakeRequest)
      contentAsString(result) mustBe councilTaxAnnex(frontendAppConfig)(fakeRequest, messages).toString
    }
  }
}