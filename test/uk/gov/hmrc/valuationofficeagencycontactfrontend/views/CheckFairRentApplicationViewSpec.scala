...
class CheckFairRentApplicationViewSpec extends ViewBehaviours {
  ...
  def checkFairRentApplication = app.injector.instanceOf[check_fair_rent_application]

  def view = () => checkFairRentApplication()(fakeRequest, messages)

  "Check Fair Rent Application view" must {
    behave like normalPage(view, "checkFairRentApplication", "title", "heading",
      "p1", "p2", "subheading", "p3")
