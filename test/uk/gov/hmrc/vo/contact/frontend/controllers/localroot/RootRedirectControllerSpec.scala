...
package uk.gov.hmrc.vo.contact.frontend.controllers.localroot
...
class RootRedirectControllerSpec extends AnyWordSpec with Matchers with GuiceOneAppPerSuite with Injecting {
  ...
  "RootRedirectController" should {
    "return redirect to start page" in {
      ...
      redirectLocation(result) shouldBe Some(uk.gov.hmrc.vo.contact.frontend.controllers.routes.Application.start().url)
    }
  }
